if (requireNamespace("box", quietly = TRUE)) {
    ...
    this.path::with_script_path(box::script_path())
    ...
    this.path::with_script_path(stopifnot(!isNamespaceLoaded("box")))
    stopifnot(!isNamespaceLoaded("box"))
    ...
    stopifnot(is.null(srcfile[[".this.path::document.context"]]))


    namespace <- attr(mod, "namespace")
    stopifnot(!is.null(namespace))
    ## we expect the document context to not exist just yet
    stopifnot(is.null(attr(namespace, ".this.path::document.context")))


    mod$fun_that_calls_src_path(verbose = FALSE)
    doc_cntxt <- srcfile[[".this.path::document.context"]]
    ...
    doc_cntxt_2 <- attr(namespace, ".this.path::document.context")
    ## now we do expect the document context to exist
    ...
    stopifnot(identical(mod$doc_cntxt, srcfile[[".this.path::document.context"]]))


    namespace <- attr(mod, "namespace")
    stopifnot(!is.null(namespace))
    ## we expect the document context to be identical to that of sys.path()
    stopifnot(identical(mod$doc_cntxt, attr(namespace, ".this.path::document.context")))
