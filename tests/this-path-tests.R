local({
    ...
    if (requireNamespace("knitr", quietly = TRUE)) {
        ...
        options(`this.path::sys.path() expectation` = normalizePath(abs_path_Rmd, "/", TRUE))
        setwd(basename_dir)
    ...
    if (requireNamespace("compiler", quietly = TRUE)) {
        ...
        options(`this.path::sys.path() expectation` = normalizePath(abs_path_Rc, "/", TRUE))
        setwd(basename_dir)
    ...
    if (requireNamespace("box", quietly = TRUE)) {
        options(`this.path::sys.path() expectation` = normalizePath(abs_path_R, "/", TRUE))
        ...
        if (!this.path:::.is_abs_path(rel_path_R)) {
            ...
            tmp <- tmp.fun(c(".", this.path::path.split.1(sub("\\.R$", "", rel_path_R))))
    ...
    if (requireNamespace("plumber", quietly = TRUE)) {
        ...
        options(`this.path::sys.path() expectation` = normalizePath(entrypoint_R, "/", TRUE))
        setwd(basename_dir)
        fun(plumber::plumb())
        setwd(rel_path_dir)
    ...
    if (requireNamespace("utils", quietly = TRUE)) {
        ...
        options(`this.path::sys.path() expectation` = normalizePath(abs_path_Rnw, "/", TRUE))
        setwd(basename_dir)
